...
$realistic_slides
  ...
  <script>
    // === ADAPTIVE: canvas dimensions scelte in base al device ===
    // Reveal scala il canvas mantenendo il rapporto e centrandolo nel
    // viewport. Scegliamo canvas dimensions ottimali per format/device,
    // cosi' il contenuto risulta leggibile su qualsiasi schermo.
    const vw = window.innerWidth, vh = window.innerHeight;
    const aspect = vw / vh;
    const isLandscape = aspect > 1.1;
    const isMobile = Math.min(vw, vh) < 500;
    ...
    // Canvas dimensions:
    // - Mobile portrait: 720x1280 (canvas piu' piccolo -> font dentro
    //   proporzionalmente piu' grandi una volta scalato sul telefono)
    // - Tablet/palco portrait: 1080x1920
    // - Mobile landscape: 1280x720
    // - Desktop landscape: 1920x1080
    let canvasW, canvasH;
    if (isLandscape) {
      canvasW = isMobile ? 1280 : 1920;
      canvasH = isMobile ?  720 : 1080;
    } else {
      canvasW = isMobile ?  720 : 1080;
      canvasH = isMobile ? 1280 : 1920;
    }
